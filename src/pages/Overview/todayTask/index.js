  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
      ...
      <div className="space-y-3">
        ...
        <div className="bg-white rounded-md shadow-sm overflow-hidden border border-gray-200">
          <div className="p-3 border-b border-gray-100">
            <h2 className="text-sm font-semibold text-gray-800">Công việc hôm nay</h2>
          </div>
          ...
          <div className="p-3">
            {loading ? (
                ...
                {todayTasks.map((task, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium truncate">{task.companyName || 'Không có tên'}</p>
                      <p className="text-xs text-gray-500 truncate">{task.connectionType || ''}</p>
                    </div>
                    
                    <span className={`ml-2 px-2 py-0.5 rounded-full text-xs flex items-center ${getStatusClassName(task.status)}`}>
              ...
              <div className="py-4 text-center">
                ...
                <p className="text-gray-500 text-xs">Không có công việc nào cho hôm nay</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-md shadow-sm overflow-hidden border border-gray-200">
          <div className="p-3 border-b border-gray-100 flex justify-between items-center">
            <h2 className="text-sm font-semibold text-gray-800">Tổng quan dự án</h2>
            ...
            {loading ? (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tên công ty</th>
                    <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Người lắp đặt</th>
                    <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ngày lắp</th>
                    <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Trạng thái</th>
                  </tr>
                </thead>
                    ...
                    <tr key={task._id || index} className="hover:bg-gray-50 transition-colors">
                      <td className="px-3 py-2 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <span className="text-xs font-medium text-gray-900 truncate max-w-[120px]">{task.companyName || 'N/A'}</span>
                        </div>
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="w-5 h-5 rounded-full overflow-hidden bg-gray-200 mr-1.5">
                            <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(task.installer || 'N/A')}&background=random&size=20`} alt={task.installer || 'N/A'} />
                          </div>
                          <span className="text-xs text-gray-600">{task.installer || 'N/A'}</span>
                        </div>
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap">
              ...
              <div className="py-6 text-center">
                ...
                <p className="text-gray-500 text-xs">Không có dữ liệu để hiển thị</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="bg-white rounded-md shadow-sm border border-gray-200 p-3">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-xs font-semibold text-gray-800">Hoạt động hàng tuần</h2>
            ...
            <div className="flex items-center">
              <div className="flex-1">
                <div className="flex items-center">
                  <span className="text-xl font-bold text-gray-800">{projectStats.weeklyActivity}%</span>
                  <span className="ml-2 px-1.5 py-0.5 bg-green-100 text-green-800 text-xs rounded-full">+12%</span>
                </div>
                <div className="mt-1.5 w-full bg-gray-200 rounded-full h-1.5">
                  <div className="bg-blue-600 h-1.5 rounded-full" style={{ width: `${projectStats.weeklyActivity}%` }}></div>
                </div>
                <p className="mt-1 text-xs text-gray-500">So sánh với tuần trước</p>
              </div>
          ...
          <div className="bg-white rounded-md shadow-sm border border-gray-200 p-3">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-xs font-semibold text-gray-800">Công việc hoàn thành</h2>
              <button className="text-gray-400 hover:text-gray-600">
                <MoreVertical size={14} />
              </button>
            </div>
            ...
            <div className="flex items-center">
              <div className="flex-1">
                <span className="text-xl font-bold text-gray-800">{projectStats.totalTasksDone}</span>
        ...
        <div className="bg-white rounded-md shadow-sm border border-gray-200 p-3">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-xs font-semibold text-gray-800">Dự án đã tham gia</h2>
            <button className="text-gray-400 hover:text-gray-600">
              <MoreVertical size={14} />
            </button>
          </div>
          ...
          <div className="flex items-center mb-2">
            <div className="flex-1">
              <span className="text-xl font-bold text-gray-800">{projectStats.projectsWorked}</span>
              <div className="mt-1.5 flex items-center">
                <span className="text-green-600 text-xs font-medium">+{Math.min(5, projectStats.projectsWorked)} dự án</span>
        ...
        <div className="bg-white rounded-md shadow-sm border border-gray-200 p-3">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-xs font-semibold text-gray-800">Thành viên nhóm</h2>
              ...
              <div key={index} className="p-0.5">
                <div className="group relative">
                  <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center cursor-pointer overflow-hidden border-2 border-white hover:border-blue-500 transition-all">
                    <img 
                      src={`https://ui-avatars.com/api/?background=c7d2fe&color=3730a3&bold=true&name=User+${index+1}&size=32`} 
                      alt={`User ${index+1}`} 
